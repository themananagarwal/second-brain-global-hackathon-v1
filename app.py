...
lookback_days = st.sidebar.number_input("Demand lookback (days)", 30, 365, 120, 10)
...
def rebuild_inventory_and_metrics():
    ...
    # Provide a SKU->weight map if available; keep empty if not needed
    sku_weights = {}
    # Note: your Modules.rolling_eoq signature is calculate_rolling_eoq(salesdf, skuweights, lookbackdays, today=None)
    eoq_df = calculate_rolling_eoq(
        sales_df, skuweights=sku_weights, lookbackdays=lookback_days
    )
    eoq_df.to_csv(EOQ_OUT_CSV, index=False)

    # Note: your Modules.reorder_evaluator signature uses snake-case names like salesfile, inventoryfile, eoqfile
    rop_df = evaluate_reorder_points(
        salesfile=SALES_XLSX,
        inventoryfile=LATEST_INV_CSV,
        eoqfile=EOQ_OUT_CSV,
        sheetname="Sheet1",
        lookbackdays=lookback_days,
        defaultleadtimedays=default_lead_time,
        defaultservicelevel=default_service,
        onorderfile=None,
        backordersfile=None,
        minsafetystock=min_safety,
        minreorderpoint=min_rop,
    )
...
if rebuild:
    ...
    rop_df = try_read_csv(REORDER_EVAL_CSV, ["Particular", "reorderpoint", "inventoryposition", "needreorder", "suggestedorderqty", "leadtimedays", "servicelevel"])
...
with k2:
    need = int(rop_df["needreorder"].sum()) if "needreorder" in rop_df.columns else 0
...
with k4:
    avg_lt = float(rop_df["leadtimedays"].mean()) if "leadtimedays" in rop_df.columns and not rop_df.empty else 0
...
with t1:
    ...
    if not rop_df.empty and {"Particular", "inventoryposition", "reorderpoint"}.issubset(rop_df.columns):
        table = rop_df[["Particular", "inventoryposition", "reorderpoint", "needreorder", "suggestedorderqty"]].sort_values(
            ["needreorder", "Particular"], ascending=[False, True]
        )
...
with t2:
    ...
    if not rop_df.empty and "needreorder" in rop_df.columns:
        suggest = rop_df[rop_df["needreorder"] == True][["Particular", "suggestedorderqty", "EOQ", "leadtimedays", "servicelevel"]]
        st.dataframe(suggest.sort_values("suggestedorderqty", ascending=False), use_container_width=True)
    else:
